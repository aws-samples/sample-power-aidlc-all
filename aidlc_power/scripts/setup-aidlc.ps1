...
try {
    ...
    if (Test-Path $PowerSteeringSrc) {
        Get-ChildItem -Path $PowerSteeringSrc -Filter "aidlc-*.md" | ForEach-Object {
            Copy-Item -Force $_.FullName (Join-Path $WorkspacePath ".kiro\steering\")
            Write-Host "    Installed steering: $($_.Name)"
        }
    }
